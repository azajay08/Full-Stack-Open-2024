...
describe("Blog app", () => {
  ...
  describe("When logged in", () => {
    ...
    test("a new blog can be created", async ({ page }) => {
      ...
      await expect(
        page.getByText("a new blog example by mr blogs added"),
      ).toBeVisible();
      await expect(
    ...
    describe("Several blogs exists", () => {
      ...
      test("only the user who added blog can delete blog", async ({
        page,
        request,
      }) => {
        const blog = page.locator(".blog").filter({ hasText: "example1" });
      ...
      test("blogs are arranged in order of likes with most likes first", async ({
        page,
      }) => {
